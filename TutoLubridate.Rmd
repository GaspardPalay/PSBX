...
si vous souhaitez installer la totalité du package tidyverse (très utile) prenez la fonction suivante: 
...
_transformation d'un vecteur de classe chaîne de caractères en un vecteur de classe date_
...
## **Calcul des périodes de temps écoulés ou de durée**
...
t0+years(38)
```
Vous remarquez que le résultat est inexacte puisqu'il prédit un évenemet le 12/09 sans compter les années bissextiles, pour obtenir le bon résultat on fait : 
```{r}
t0+dyears(38)
```
...
leap_year(2017)
```
## **Calculs artithméiques**

Non pouvons inclure des calculs arithmétiques sur des périodes ou des durées. 
Les périodes correspondent au fonctions **xxx()** (par exemple **days()** ou **months()**) tandis que les durrées correspondent aux fonctions **dxxx()** (par exemple **ddays()** ou **dyears()**)
```{r}
